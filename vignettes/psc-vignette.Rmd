...
which differes from the Average Treatment Effect (ATE) obtained in other settings 
and adresses the question of whether treatments are effective in the population 
...
head(surv.post)
```
Inspection will show that there is a column for each parameter in the original 
model as well as 'beta' and 'DIC' columns which give the posterior estimates 
for $\beta$ and the Deviance Informaiton Criterion respectively.
...
and estimation procedure that is required.  In this example, the "enrichwith" 
